pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '992382702050.dkr.ecr.ap-northeast-2.amazonaws.com/eks-demo-repo'
    awsecrRegistryCredential = 'credential-AWS-ECR'
    githubCredential = 'credential-github'
    gitEmail = 'spqj0409@gmail.com'
    gitName = 'cozymh'
  }
  ...
  stages {
    ...
    stage('Checkout Application Git Branch') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/cozymh/kustomize.git']]])
      }
      // steps 가 끝날 경우 실행한다.
    ...
    stage('Deploy') { 
      steps {
        ...
        git credentialsId: githubCredential,
            url: 'https://github.com/cozymh/kustomize.git',
            branch: 'main'  
 ...
 stage('Push to Git Repository') {
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: githubCredential, usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD']]) {
             sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/cozymh/kustomize.git"
        }
      }
  }  
 }
